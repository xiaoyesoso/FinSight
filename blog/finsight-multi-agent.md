# 用 Claude Agent SDK 搭一个多代理投研平台，从 CLI 脚本到全链路可观测
...
financial_analyzer_agent = AgentDefinition(
    description="财务报告分析专家",        # 主代理靠这个描述决定何时调用
    prompt="你是一位资深财务分析师...",     # 子代理的系统提示词
    tools=["Read", "Grep", "Glob"],       # 这个子代理能用的工具
    skills=["financial-report-analyzer"],  # 挂载的 Skill（黑盒复用）
    model="inherit",                       # 继承主代理的模型配置
)
```
...
这个隔离特别重要。我在 GitHub 上翻到 Anthropic 自己的内部测试数据，多代理架构相比单代理，任务完成率提升了 **90.2%**，代价是 token 消耗大约是 15 倍。听起来成本很高，但你想想，三个子代理并行跑，每个都在自己的领域深耕，比起一个代理在三个领域之间来回切换角色，质量上的提升远超 token 的代价。
...
options = ClaudeAgentOptions(
    session_id=str(uuid.uuid4()),  # 显式传入 UUID，SDK 会持久化到磁盘
...
options = ClaudeAgentOptions(
    ...
    agents=agents_config,          # 显式传入 agents（SDK 不一定可靠恢复）
    # hooks 和 env 一样要重新传入
)

# fork, 分叉探索，克隆出新 session_id，原会话不受影响
options = ClaudeAgentOptions(
    resume=session_id,             # 基于哪个会话分叉
    fork_session=True,             # 关键, 触发克隆
    max_turns=5,                   # 限制分叉深度，避免无限探索
...
```python
# 从 SDK 的最终消息中提取 session_id
async for msg in client.receive_response():
    if isinstance(msg, ResultMessage):
        session_id = msg.session_id  # fresh/fork 是新 ID, resume 是原 ID
        # 附在 final_result 事件里返回给前端
        yield {
            "type": "final_result",
            "data": {
                "report": aggregated_report,
                "session_id": session_id,
                # fork 模式下还有 parent_session_id
...
这就是 Hook 机制的价值。SDK 提供了 18 个生命周期 Hook，在工具调用前后、子代理启停、会话结束等节点都会触发回调。我用其中几个搭建了三道防线。
...
```python
def pre_tool_guard(hook_input: PreToolUseHookInput) -> dict:
    """PreToolUse hook, 自动审批只读工具，拦截高危操作。"""
    tool = hook_input["tool_name"]

    if tool in READ_ONLY_TOOLS:
        # 只读工具自动放行
        # 注意, permissionDecision 必须嵌套在 hookSpecificOutput 中
        # 放在顶层会被 CLI 静默忽略
        return {
            "hookSpecificOutput": {
    ...
    if _is_dangerous(tool, hook_input["tool_input"]):
        return {
            "hookSpecificOutput": {
                "hookEventName": "PreToolUse",
                "permissionDecision": "deny",
                "permissionDecisionReason": f"blocked: {tool} on sensitive path",
            }
        }

    return {}  # 空字典表示不干预，走默认流程
```

这里有个巨坑。`permissionDecision` 字段必须嵌套在 `hookSpecificOutput` 里面，不能直接放在返回字典的顶层。我一开始按照直觉写了顶层，结果 CLI 完全无视我的决策，该放行的没放行，该拦截的没拦截。查了 SDK 的 `SyncHookJSONOutput` 类型定义才发现嵌套结构的要求。

**第二道防线，PostToolUse 审计日志。** 每次工具调用后触发，把调用的完整信息写入 JSONL 文件。这样每一份投研报告都可以回溯到它调用了哪些工具、读了哪些文件、搜了什么关键词。

```python
def post_tool_audit(hook_input: PostToolUseHookInput) -> None:
    """PostToolUse hook, 把每次工具调用记录到审计日志。"""
    record = {
        "ts": datetime.now(timezone.utc).isoformat(),
        "session_id": hook_input["session_id"],
        "agent_id": hook_input.get("agent_id", "orchestrator"),
        "tool": hook_input["tool_name"],
        "input": hook_input["tool_input"],
        # 工具输出可能很大（文件内容、搜索结果），只存前 500 字符
        "output_summary": str(hook_input["tool_response"])[:500],
    }
...
```python
def build_otel_env(
    service_name: str = "finsight",
    enduser_id: str = "",    # 分析师 ID，用于按人归因成本
    tenant_id: str = "",     # 租户/团队 ID，用于按团队分摊
) -> dict[str, str]:
    """构造 OTel 环境变量，未配置端点时返回空字典（opt-in 设计）。"""
    endpoint = settings.otel_exporter_otlp_endpoint
    if not endpoint:
        return {}  # 没配端点就禁用遥测，不影响正常运行

    # 资源属性, 会附在每条 span/metric/log 上
    attrs = [
        "service.version=1.0.0",
        "deployment.environment=development",
    ]
    if enduser_id:
        # percent-encode, 防止逗号/等号破坏 OTEL_RESOURCE_ATTRIBUTES 解析
    ...
    return {
        ...
        # 三种信号各选 otlp exporter
        "OTEL_TRACES_EXPORTER": "otlp",
        "OTEL_METRICS_EXPORTER": "otlp",
        "OTEL_LOGS_EXPORTER": "otlp",
        # OTLP 传输协议, 用 HTTP 而非 gRPC（少一个依赖）
        "OTEL_EXPORTER_OTLP_PROTOCOL": "http/protobuf",
        "OTEL_EXPORTER_OTLP_ENDPOINT": endpoint,
        # 服务名, 在 Jaeger/Grafana 里按这个筛选
...
- **C**，单次调用总成本（美元）
- **Ni**，输入 token 数，即发送给模型的 prompt + 历史 + 工具结果的总 token 量
- **Pi**，输入 token 单价（美元/百万 token），不同模型价格不同
- **No**，输出 token 数，模型生成的文本量
- **Po**，输出 token 单价，通常比输入贵 3-5 倍
- **Nc**，缓存命中 token 数，prompt caching 命中后读取的 token 量
- **Pc**，缓存读取单价，通常是标准输入价格的 10-20%（便宜 80-90%）
- **1,000,000**，因为单价是「每百万 token」，所以要除以一百万

多代理场景下，一次投研任务的总成本是所有子代理和主代理各自调用的 C 之和。这就是为什么 token 消耗会是单代理的 15 倍，三个子代理各有自己的输入输出，主代理还要汇总，token 走的是加法。但缓存命中率高的话，实际花费会低很多。OTel 的 metrics 里会自动记录 `cache_read_input_tokens`，你在 Grafana 里可以做一个 `cached_vs_uncached` 的对比图来看缓存效果。注意值里如果有逗号、空格、等号，必须先做 percent-encode，否则会破坏 `OTEL_RESOURCE_ATTRIBUTES` 的解析格式。
